...
const WebmentionReplies = ({ target }) => {
  ...
  const fetchMentions = async (target) =>
    fetch(
      `https://webmention.io/api/mentions.jf2?per-page=500&target=${encodeURIComponent(target)}&token=${CONFIG.COMMENT_WEBMENTION.TOKEN}`
...
const WebMentionBlock = ({ frontMatter }) => {
  ...
  const url = `https://${CONFIG.COMMENT_WEBMENTION.HOSTNAME}${router.asPath}`
  const tweet = `${frontMatter.title} by @${CONFIG.COMMENT_WEBMENTION.TWITTER_USERNAME} ${url}`
