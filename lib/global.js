...
export function GlobalContextProvider({ children }) {
  const [lang, updateLang] = useState(CONFIG.LANG)
  const [locale, updateLocale] = useState(generateLocaleDict(CONFIG.LANG))
  const [theme, setTheme] = useState(CONFIG.THEME)
  const [isDarkMode, updateDarkMode] = useState(CONFIG.APPEARANCE === 'dark')
