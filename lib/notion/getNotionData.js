...
export async function getGloablNotionCollectionData({ pageId, from }) {
  const notionPageData = deepClone(await getNotionCollectionData({ pageId, from }))
...
export async function getNotionCollectionData({ pageId, from }) {
  const cacheKey = 'page_block_' + pageId
  return await getOrSetFromCache(cacheKey, async () => {
    return await getCollectionData({ pageId, from })
  })
}
...
function getCustomMenu({ collectionData }) {
  const menuPages = collectionData.filter(post => (post.type === CONFIG.NOTION_PROPERTY_NAME.type_menu || post.type === CONFIG.NOTION_PROPERTY_NAME.type_sub_menu) && post.status === 'Published')
  const menus = []
  if (menuPages && menuPages.length > 0) {
    menuPages.forEach(e => {
      e.show = true
      if (e.type === CONFIG.NOTION_PROPERTY_NAME.type_menu) {
        menus.push(e)
      } else if (e.type === CONFIG.NOTION_PROPERTY_NAME.type_sub_menu) {
...
async function getCollectionData({ pageRecordMap, pageId, from }) {
  if (!pageRecordMap) {
...
async function getPageRecordMapByNotionAPI({ pageId, from }) {
  ...
  const { collectionData, schema, collection, pageIds } = await getCollectionData({ pageRecordMap, pageId })

  // 文章计数
  let postCount = 0
  // 查找所有的Post和Page
  const allPages = collectionData.filter(post => {
    if (post.type === CONFIG.NOTION_PROPERTY_NAME.type_post && post.status === CONFIG.NOTION_PROPERTY_NAME.status_publish) {
      postCount++
    }
