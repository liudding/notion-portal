...
export async function getGlobalNotionData({
  pageId = BLOG.WEBSITE_NOTION_PAGE_ID,
  from
}) {
  // 获取Notion数据
...
function getBlogInfo({ collection, block }) {
  ...
  const pageCover = collection?.cover ? (mapImgUrl(collection?.cover, block[idToUuid(BLOG.WEBSITE_NOTION_PAGE_ID)]?.value)) : BLOG.HOME_BANNER_IMAGE
