...
export default async function getPageProperties(id, block, schema, authToken, tagOptions) {
  ...
  if (value.created_time) {
    properties.createdTime = formatDate(new Date(value.created_time).toString(), CONFIG.LANG)
  }

  if (value.last_edited_time) {
    properties.lastEditedTime = formatDate(new Date(value.last_edited_time).toString(), CONFIG.LANG)
  }
