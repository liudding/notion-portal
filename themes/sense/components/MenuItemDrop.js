...
export const MenuItemDrop = ({ link }) => {
  ...
  return <li onMouseOver={() => changeShow(true)} onMouseOut={() => changeShow(false)}
    className='relative py-1 duration-500 justify-between text-gray-500 dark:text-gray-300 hover:text-black hover:underline cursor-pointer flex flex-nowrap items-center text-right'>

    {!hasSubMenu &&
      <Link href={link?.to} className='w-full my-auto items-center justify-between flex flex-row-reverse' >
    ...
    {hasSubMenu && <ul className={`${show ? 'visible opacity-100 left-52' : 'invisible opacity-0 left-40'} py-1 absolute right-0 top-0 w-full border-gray-100  bg-white  dark:bg-black dark:border-gray-800 transition-all duration-300 drop-shadow-lg `}>
      {link?.subMenus?.map(sLink => {
        return <li key={sLink.id}>
          <Link href={sLink.to} className='my-auto py-1 px-2 items-center justify-start flex flex-row-reverse text-gray-500 dark:text-gray-300 hover:text-black  tracking-widest transition-all duration-200 dark:border-gray-800 '>
            {sLink.icon && <i className={`${sLink.icon} w-4 text-center `} />}
            <div className={'ml-2 whitespace-nowrap'}>{sLink.name || sLink.title}</div>
            {sLink.slot}
          </Link>
        </li>
      })}
