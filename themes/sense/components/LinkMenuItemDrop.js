...
export const LinkMenuItemDrop = ({ link }) => {
  ...
  return <li
    ...
    <Scroll.Link onMouseUp={() => changeShow(!show)} to={link?.id} spy={true} smooth={true} offset={-50} duration={500} className='w-full my-auto items-center rounded p-2 hover:bg-gray-100 justify-between flex' >
      ...
      <ul className={`${show ? 'block visible opacity-100' : 'hidden invisible opacity-0'} py-1 w-full border-gray-100 list-none dark:border-gray-800 transition-all duration-300`}>
        {link?.subMenus?.map(sLink => {
          return <li key={sLink.id}>
            <Scroll.Link to={sLink.id} spy={true} smooth={true} offset={-50} duration={500} className='my-auto p-2 rounded items-center justify-start flex text-gray-500 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-900 tracking-widest transition-all duration-200 dark:border-gray-800 '>
