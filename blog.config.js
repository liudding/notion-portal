...
const BLOG = {
  ...
  NOTION_PAGE_ID: process.env.NOTION_PAGE_ID || 'dd6ceea36f244c23afdd6380d1bb781b',
  LINKS_NOTION_PAGE_ID: process.env.NOTION_PAGE_ID || '2274bcbbab7b4cd6a5b9c7b2876afda8',

  LINKS_CATEGORY_LEVELS: ['category1', 'category2'],

  PSEUDO_STATIC: false, // 伪静态路径，开启后所有文章URL都以 .html 结尾。
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 60000, // 更新内容缓存间隔 单位(秒)；即每个页面有5秒的纯静态期、此期间无论多少次访问都不会抓取notion数据；调大该值有助于节省Vercel资源、同时提升访问速率，但也会使文章更新有延迟。
  THEME: process.env.NEXT_PUBLIC_THEME || 'sense', // 主题， 支持 ['next','hexo',"fukasawa','medium','example'] @see https://preview.tangly1024.com
